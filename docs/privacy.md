...
- The on-device ASR path is the exception: it writes the recording to `~/Library/Application Support/VibeVoiceOSS/NativeASR/jobs/<uuid>/recording.wav` and deletes it once transcription finishes. A crash or force-quit mid-transcription can leave the file behind.
