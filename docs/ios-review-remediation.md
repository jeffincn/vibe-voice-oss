...
| H10 | The keyboard extension had no runtime observability at all, so every degraded path — App Group missing, librime falling back, a host dropping an insertion — was indistinguishable from "nothing happened" | fixed | `feat(ios): make the keyboard observable on a real device`. See `docs/ios-device-debugging.md`. |
| H11 | `currentDocumentID` was read but never assigned, so `delivery` always returned `.awaitExplicitInsert` and a transcript never auto-inserted. The assignment existed once and crashed the extension: `documentIdentifier` is declared non-null and returns nil, trapping in `UUID._unconditionallyBridgeFromObjectiveC` (`VibeVoiceKeyboard-2026-07-29-141238.ips`) | fixed | `feat(ios): make the keyboard observable on a real device`. Read through KVC so nil stays representable. |
| H12 | Both device scripts took the first `Apple Development` identity in the keychain. On a Mac signed into several Apple IDs that is the wrong team, and the build fails with `No profiles for 'app.vibevoice.oss.ios'` — which reads as a missing profile rather than a wrong team | fixed | `feat(ios): make the keyboard observable on a real device`. `scripts/ios-signing.zsh` derives the team from a profile that already covers the bundle ID, then picks the matching identity. |
