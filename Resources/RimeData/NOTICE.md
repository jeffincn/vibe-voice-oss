...
`key_bindings.yaml` is from
[`rime/rime-prelude`](https://github.com/rime/rime-prelude) (also shipped with
Squirrel) and is distributed under the BSD 3-Clause License, same as librime.

`default.yaml` and `vibe_pinyin.schema.yaml` are original Vibe Voice OSS
configuration files; their `ascii_composer` / `key_binder` / `recognizer`
sections follow Squirrel's SharedSupport defaults.
